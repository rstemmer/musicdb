...
This module provides a thread that takes the videos from a :class:`musicdb.mdbapi.videoqueue.VideoQueue` to stream them.
...
def StartVideoStreamingThread(config, musicdb):
    """
    This function starts the Streaming Thread :meth:`~VideoStreamingThread`.
...
def VideoStreamingThread():
    ...
    Commands:
        ...
        * ``VideoEnded``: When a client send the VideoEnded information to the server (:meth:`~musicdb.lib.ws.mdbwsi.MusicDBWebSocketInterface.VideoEnded` this thread manages the call. In case the provided entry ID is the same as the ``currententy`` ID the next video gets popped from the video queue. Otherwise the command gets ignored.
...
class VideoStreamManager(object):
    ...
    .. note::
        ...
        **Important for developer:**
        ...
        Never call the :meth:`musicdb.mdbapi.videoqueue.VideoQueue.NextVideo` method directly from this class!
    ...
    def Play(self, play=True):
        ...
        the :meth:`~musicdb.mdbapi.videostream.VideoStreamingThread` will trigger the ``StatusChanged`` event.
