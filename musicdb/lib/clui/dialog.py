...
class Dialog(ListView, ButtonView):
    ...
    Pressing the space bar on an :class:`~musicdb.lib.clui.boolinput.BoolInput` toggles its value.
    On enter (↵), the changes shoud be committed, on escape (␛) they should be rejected.

    Beside the list of input elements a :class:`~musicdb.lib.clui.buttonview.ButtonView` gets printed at the bottom of
    ...
    def AddInput(self, label, control, hint=None):
        ...
        The following inputs are suitable for the dialog:
            * :class:`musicdb.lib.clui.textinbut.TextInput`
            * :class:`musicdb.lib.clui.boolinput.BoolInput`
